...
if (li>0) then
			for i = 29,0,-1 do -- parcourir les 15 sessions en commencant par 14

				 for j = 1,4 do -- copier par bloc de 26 carac (4*26 = 104 : sequence complete )

					local file = io.open(fich1, "r") -- ouvrir fichier 0-log.txt en acces lecture
					local curs = io.seek(file,1+(j-1)*26 + i*104 ) --  positionner curseur dans fichier log
					local temp = io.read (file, 26) -- lire 26 carac de  dans fichier log et asssigner a temp
					io.close(file) -- fermer fichier log
					
					local file = io.open(fich1, "a") -- ouvrir fichier 0-log.txt en acces ecriture et en preservant son contenu
					local curs = io.seek(file, 1+(j-1)*26 + 104 + i*104) --  positionner curseur dans fichier log decaler a session suivante
...
for i = 1,4 do
local ecri = io.write (file, "xxxxxxxxxxxxxxxxxxxxxxxxxx") -- remplir de x carte sd session 1 par bloc de 26
end
...
-- shared.var01: variable affecté en permanence quand nimporte quel ecran est affiché: 
-- et accessible en permanence depuis nimporte quel ecran
-- shared.var01 = test  -- test
...
if ( sc > 1 and (getTime() - lapold) > 600 )  then-- si appui bouton tps tour et superieur a 6 seconde depuis dernier appui
...
lapold = getTime()
		...
		-- copie lap sur carte sd :
				...
				if (num<12) then -- si tour sup a 11 on nenregistre plus sur sd
...
if (getTime()> (5*100+ stock) ) then -- A FAIRE TOUTE LES :    5*100 x 10ms = 5s
	...
	-- ======== FORMATAGE VALEUR SENSOR ==============
		...
		tim1 = string.format("%06d", tim1) -- formater 4 carac
