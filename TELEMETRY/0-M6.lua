...
for compte in dir("/LOGS") do -- lister fichier dans LOGS
    z = z+1
		if string.sub(compte,1,1) == item then -- trie seulement fichier log correspondant au model en cours
		debut = debut + 1 -- nombre de fichier log
		modif= z -- index dernier fichier log
		end
end

if debut > 30 then -- si pluss de 30 fichier log
nbfile = 30 -- recup les 30 dernier log
debut = modif - 29 -- index debut fichier log
else
nbfile = debut -- nombre de log
debut = modif - nbfile + 1 -- index debut fichier log
end

z = 0 

for fname in dir("/LOGS") do -- lister fichier dans LOGS
        if rot >= debut and rot <= modif then
...
rot = 1 -- numero item a modifier (commande par rotary)
modif = 0 --   =0 si pas mode modif et =valeur de rot si modif de cet item 
